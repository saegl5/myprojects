...
def walk(count, sprite, sprite_sheet, frame_list, width, height, facing_left):
    if count == 1: # in case there is a quick KEYDOWN and KEYUP event
        ...
        sprite.image = pygame.Surface((frame_list[1][2], frame_list[1][3]), pygame.SRCALPHA).convert_alpha()
    ...
    if count == 5: # else mario appears to hover
        sprite.rect.w = frame_list[1][2] # align right edge to other sprites, don't use if walk in midair
        sprite.image = pygame.Surface((frame_list[1][2], frame_list[2][3]), pygame.SRCALPHA).convert_alpha() # widest, equally
...
def stand(sprite, sprite_sheet, frame_list, width, height, facing_left):
    ...
    sprite.image = pygame.Surface((frame_list[0][2], frame_list[0][3]), pygame.SRCALPHA).convert_alpha()
