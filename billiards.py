...
ball_picture = pygame.image.load('ball_blue_large.png') # foreground picture from https://kenney.nl/assets/rolling-ball-assets, see License.txt; the ball is a circle, but the picture of it is a rectangle
background_picture = pygame.image.load('background_green.png') # background picture from https://kenney.nl/assets/rolling-ball-assets, see License.txt
...
while True: # keeps display open
    ...
    # --- Drawing code
    for i in range(0, size[0], 64): # 64 pixels is step size, based on width of background picture
        for j in range(0, size[1], 64): # again, 64 pixels is step size, but this one is based on height of background picture
            screen.blit(background_picture, (i, j)) # copy the background picture onto the screen, fills in vertically from left to right
    ...
    screen.blit(ball_picture, (0+x_offset, 0+y_offset)) # copy the foreground picture onto the screen starting at (0+x_offset, 0+y_offset)
    ...
    if 0+y_offset + 64 > size[1]: # if rectangle would otherwise breach bottom edge
        # increment *= -1 # increment = increment*-1, that is change the increment's sign
        y_offset = size[1] - 64 # prevent rectangle from breaching edge
    ...
    if 0+x_offset + 64 > size[0]: # if rectangle would otherwise breach right edge
        x_offset = size[0] - 64 # prevent rectangle from breaching edge
