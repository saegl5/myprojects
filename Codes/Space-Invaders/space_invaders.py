...
class Rectangle(pygame.sprite.Sprite): # make Rectangle class of same class as sprites, use sentence case to distinguish class from a function
    def __init__(self, sprite_image, W, H): # define a constructor, class accepts width, height, and image parameters, must type "__" before and after "init," requires "self"
...
for i in range(0, 50): # FOR fifty indices (i.e., each index between 0 and, but not including, 50), create and add fifty "block" sprites
    ...
    block.rect.y = random.randrange(0, size[1]+1-H/2-96, H/2) # "-96" leaves space at bottom of canvas and want "block" sprites equally spaced, also mitigates overlap
        ...
        # --- Mouse/keyboard events
            ...
            if timer != 0 and len(blocks) != 0 and len(players) != 0:
                if action.key == pygame.K_RIGHT: # note "action.key"
                    ...
                    laser = Rectangle(pygame.Surface((4, 20)), 4, 20) # create "laser" sprite 
