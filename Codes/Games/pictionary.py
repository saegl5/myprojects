...
while not done:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            draw = True # replaced drawing code with this
        elif event.type == pygame.MOUSEBUTTONUP: # new
            draw = False # new
        else:
            None
    ...
    if draw == True: # IF mouse/trackpad button pressed
        draw_circle(size[0]/2+x_offset, size[1]/2+y_offset, 1) # tabbed
    else:
        None # do not draw
    pygame.display.flip()
    clock.tick(120) # fewer dots
pygame.quit()
