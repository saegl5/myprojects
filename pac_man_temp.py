...
red_ghost_image = pygame.image.load('red_ghost.png').convert()
green_ghost_image = pygame.image.load('green_ghost.png').convert()
...
wall = Rectangle(10, size[1]-100-10-10-100)
wall.rect.x = size[0]/2-10/2
wall.rect.y = 100+10
walls.add(wall)

wall = Rectangle(1, size[1]) # width of at least one pixel
wall.rect.x = 0-1
wall.rect.y = 0
walls.add(wall)

wall = Rectangle(1, size[1])
wall.rect.x = size[0]-1+1 # -1 because wall has width of 1 pixel
wall.rect.y = 0
walls.add(wall)

wall = Rectangle(size[0]-2, 1)
wall.rect.x = 1 # no overlap
wall.rect.y = 0-1
walls.add(wall)
...
while 50-len(pellets) > 0:
    ...
    for wall in walls:
        pygame.sprite.spritecollide(wall, pellets, True) # remove any "block" sprite in list in same position
    pellets.add(pellet) # not pellets.append(pellet) <-- multiple sprites

while True:
    red_ghost = Rectangle(W_ghost, H_ghost) # should write first
    red_ghost.image.blit(red_ghost_image, (0, 0))
    red_ghost.rect.x = random.randrange(0, size[0]+1-W_ghost, W_ghost)
    red_ghost.rect.y = random.randrange(0, size[1]+1-H_ghost, H_ghost)    
...
while True:
    green_ghost = Rectangle(W_ghost, H_ghost)
    green_ghost.image.blit(green_ghost_image, (0, 0))
    green_ghost.rect.x = random.randrange(0, size[0]+1-W_ghost, W_ghost)
    green_ghost.rect.y = random.randrange(0, size[1]+1-H_ghost, H_ghost)
...
while True:
    for action in pygame.event.get():
        if action.type == pygame.QUIT:
            ...
            if timer == 0:
                ...
                timer -= 1
                if timer % 5 == 0:
                    x_increment_red_ghost = random.randint(-1, 1)
    ...
    red_ghost.rect.x += x_increment_red_ghost
    ...
    red_ghost.rect.y += y_increment_red_ghost
    hit = pygame.sprite.spritecollide(red_ghost, walls, False)
    if hit:
